...
- `environment` (String) The cloud environment to use. Supported values are `public` (also `global`), `usgovernment` (also `usgovernmentl4`), `usgovernmentl5` (also `dod`), `germany` (also `german`), and `china`. This can also be sourced from the `ARM_ENVIRONMENT` Environment Variable. Defaults to `public`.
