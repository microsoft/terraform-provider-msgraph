...
- `headers` (Map of String) A mapping of HTTP headers to be sent with the update request. Note that authentication headers are automatically handled.
...
- `method` (String) The HTTP method to use for the update request. Common methods include `GET`, `POST`, `PATCH`, `DELETE`, and `PUT`.
