	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			"environment": schema.StringAttribute{
				Optional:            true,
				MarkdownDescription: "The cloud environment to use. Supported values are `public`, `china`, and `usgovernment`. This can also be sourced from the `ARM_ENVIRONMENT` Environment Variable. Defaults to `public`.",
			},
...
func (p *MSGraphProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	environment, err := ResolveCloudEnvironment(model.Environment.ValueString())
	if err != nil {
